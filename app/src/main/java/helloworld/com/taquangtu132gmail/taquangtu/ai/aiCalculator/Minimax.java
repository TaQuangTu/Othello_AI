...
public class Minimax
{
    ...
    int minimax(boolean isComputer, int depth, int maxDepth)
    {
        //if finish
        if(isFinishState())
        {
        ...
        //if not finished but has reached maximum depth
        //count sum of each type and give an estimate number
        // let's mofify this heuristic evaluation function to get the best move
        if(depth==maxDepth)
        {
            int sumOfWhite = 0;
            int sumOfBlack = 0;
            ...
            for(int i=0;i<row;i++)
            {
                for(int j=0;j<column;j++)
                {
                    if(isLegal(i,j,isComputer))
                    {
                        ...
                        put(false,i,j);
                        best = Math.min(best, minimax(!isComputer, depth+1, maxDepth));
                        ...
                        if(isLegal(i,j,isComputer))
                        {
                            ...
                            for (int ii = 0; ii<row; ii++)
                                for (int jj = 0; jj < column; jj++)
                                    t.get(ii).add(chessColorMatrix.get(ii).get(jj));
                            put(true,i,j);
                            best = Math.max(best, minimax(!isComputer, depth + 1, maxDepth));
                            //undo chessColorMatrix
                            chessColorMatrix = t;
                        }
                    }
                ...
                if(isLegal(i,j,true))
                {
                    ...
                    for (int ii = 0; ii<row; ii++)
                        for (int jj = 0; jj < column; jj++)
                            t.get(ii).add(chessColorMatrix.get(ii).get(jj));
                    put(true,i,j);
                    int moveVal = minimax(true,0, maxDepth);
                    if(best < moveVal)
                    {
                        best = moveVal;
                        position = i*column + j;
